...
import { calculatePrices, crc, DEFAULT_SETTINGS, normalizeName, PricingSettings, ProductRecord, usd } from "@/lib/pricing";
...
function Results({ price, weight, settings }: { price: number; weight: number; settings: PricingSettings }) {
...
function parseNumber(value: unknown) {
  ...
  if (clean.includes(",")) return Number(/^\d{1,3}(,\d{3})+$/.test(clean) ? clean.replace(/,/g, "") : clean.replace(",", "."));
  return Number(clean);
}

function ImportView({ settings, afterImport }: { settings: PricingSettings; afterImport: () => void }) {
  const input = useRef<HTMLInputElement>(null);
  const [fileName, setFileName] = useState(""); const [headers, setHeaders] = useState<string[]>([]); const [rows, setRows] = useState<unknown[][]>([]);
  ...
  const mapped = useMemo(() => rows.map((row, i) => ({
    rowNumber: i + 2, name: mapping.name === "" ? "" : String(row[Number(mapping.name)] ?? "").trim(),
    purchasePriceUsd: mapping.purchasePriceUsd === "" ? Number.NaN : parseNumber(row[Number(mapping.purchasePriceUsd)]),
    weightLb: mapping.weightLb === "" ? Number.NaN : parseNumber(row[Number(mapping.weightLb)]),
    code: mapping.code === "" ? "" : String(row[Number(mapping.code)] ?? "").trim(),
  })), [mapping, rows]);
  const valid = mapped.filter((row) => row.name && Number.isFinite(row.purchasePriceUsd) && row.purchasePriceUsd >= 0 && Number.isFinite(row.weightLb) && row.weightLb > 0);
  ...
  async function pick(event: ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const data = XLSX.utils.sheet_to_json<unknown[]>(book.Sheets[book.SheetNames[0]], { header: 1, defval: "", raw: true });
      if (data.length < 2) throw new Error("El archivo no contiene productos.");
      const nextHeaders = data[0].map((cell, i) => String(cell || `Columna ${i + 1}`).trim()); const normalized = nextHeaders.map(normalizeName);
      const find = (...tests: RegExp[]) => { const i = normalized.findIndex((head) => tests.some((test) => test.test(head))); return i < 0 ? "" : String(i); };
      setFileName(file.name); setHeaders(nextHeaders); setRows(data.slice(1).filter((row) => row.some((cell) => String(cell).trim())));
      setMapping({ name: find(/nombre/, /producto/, /descripcion/), purchasePriceUsd: find(/precio.*compra/, /costo.*usd/, /^precio$/), weightLb: find(/peso/, /libras/, /^lb$/), code: find(/codigo/, /barra/, /barcode/, /^qr$/) });
  ...
  async function importRows() {
    ...
    try {
      const result = await json<{ imported: number; updated: number; skipped: number; errors: unknown[] }>(await fetch("/api/products/import", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rows: mapped, strategy }) }));
  ...
  const reset = () => { setHeaders([]); setRows([]); setFileName(""); setNotice(null); };
  return <div className="view"><header className="view-head"><span className="eyebrow">Carga masiva</span><h1>Importar Excel</h1><p>Cargá tus productos y revisalos antes de guardar.</p></header>
    {!headers.length ? <section className="surface upload" onClick={() => input.current?.click()}><div className="upload-icon"><FileSpreadsheet /></div><h2>{busy ? "Leyendo archivo…" : "Seleccioná tu archivo"}</h2><p>.xlsx, .xls o .csv</p><button className="btn primary" disabled={busy}><Upload size={19} />Elegir archivo</button><input ref={input} type="file" accept=".xlsx,.xls,.csv" onChange={pick} hidden /></section> : <>
      <section className="surface file-row"><div><FileSpreadsheet /><span><b>{fileName}</b><small>{rows.length} filas</small></span></div><button className="btn ghost small" onClick={reset}><RotateCcw size={16} />Cambiar</button></section>
      ...
      <section className="surface section"><Step n="2" title="Vista previa" text={complete ? `${valid.length} de ${mapped.length} filas listas` : "Completá las columnas obligatorias."} /><div className="table-wrap"><table><thead><tr><th>Producto</th><th>Compra</th><th>Peso</th><th>GAM</th><th>Puerto</th><th>Estado</th></tr></thead><tbody>{mapped.slice(0, 6).map((row) => {
        const ok = Boolean(row.name) && Number.isFinite(row.purchasePriceUsd) && row.purchasePriceUsd >= 0 && Number.isFinite(row.weightLb) && row.weightLb > 0; const prices = calculatePrices(row.purchasePriceUsd, row.weightLb, settings);
        return <tr key={row.rowNumber}><td><b>{row.name || "Sin nombre"}</b>{row.code && <small>{row.code}</small>}</td><td>{Number.isFinite(row.purchasePriceUsd) ? usd(row.purchasePriceUsd) : "—"}</td><td>{Number.isFinite(row.weightLb) ? `${row.weightLb.toFixed(2)} lb` : "—"}</td><td>{ok ? crc(prices.gamPriceCrc) : "—"}</td><td>{ok ? crc(prices.puertoPriceCrc) : "—"}</td><td><span className={`pill ${ok ? "ok" : "bad"}`}>{ok ? <Check size={12} /> : <AlertCircle size={12} />}{ok ? "Lista" : "Revisar"}</span></td></tr>;
      })}</tbody></table></div></section>
      <section className="surface section"><Step n="3" title="Productos repetidos" text="Elegí qué hacer si el nombre o código ya existe." /><div className="strategies"><label className={strategy === "update" ? "chosen" : ""}><input type="radio" checked={strategy === "update"} onChange={() => setStrategy("update")} /><span><b>Actualizar existentes</b><small>Reemplaza precio y peso.</small></span></label><label className={strategy === "skip" ? "chosen" : ""}><input type="radio" checked={strategy === "skip"} onChange={() => setStrategy("skip")} /><span><b>Omitir existentes</b><small>Conserva los datos guardados.</small></span></label></div><button className="btn primary full" disabled={!complete || !valid.length || busy} onClick={importRows}>{busy ? <Loader2 className="spin" /> : <Upload />}Importar {valid.length || ""} productos</button></section>
...
export function NutriPlusApp() {
  ...
  const loadProduct = (product: ProductRecord) => { setForm({ id: product.id, name: product.name, code: product.code || "", purchasePriceUsd: String(product.purchasePriceUsd), weightLb: String(product.weightLb) }); setSuggestionsOpen(false); setTab("calculator"); };
  const price = Number(form.purchasePriceUsd), weight = Number(form.weightLb); const validForm = Boolean(form.name.trim()) && Number.isFinite(price) && price >= 0 && Number.isFinite(weight) && weight > 0;
  async function save(event: FormEvent) { event.preventDefault(); if (!form.name.trim()) return notify({ type: "error", text: "El nombre del producto es obligatorio." }); setSaving(true); try { const data = await json<{ product: ProductRecord }>(await fetch(form.id ? `/api/products/${form.id}` : "/api/products", { method: form.id ? "PUT" : "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ ...form, purchasePriceUsd: price, weightLb: weight }) })); const editing = Boolean(form.id); loadProduct(data.product); notify({ type: "success", text: editing ? "Producto actualizado." : "Cotización guardada." }); await fetchProducts(query); } catch (error) { notify({ type: "error", text: error instanceof Error ? error.message : "No se pudo guardar." }); } finally { setSaving(false); } }
  ...
  return <main className="app-shell"><header className="topbar"><div><button className="brand" onClick={() => setTab("calculator")}><span className="brand-mark">N+</span><span><b>NutriPlus</b><small>Calculadora de precios</small></span></button><div className="rate"><small>Tipo de cambio</small><b>₡{settings.exchangeRateCrc}</b></div></div></header>
    <div className="body"><aside className="side"><span>Menú</span>{nav.map(([id, label, Icon]) => <button className={tab === id ? "active" : ""} onClick={() => setTab(id)} key={id}><Icon />{label}</button>)}<div className="weight-note"><Weight /><span><b>+{settings.extraWeightLb.toFixed(2)} lb</b><small>en cada cálculo</small></span></div></aside><div className="content">
      {tab === "calculator" && <div className="view"><header className="view-head row"><div><span className="eyebrow">Cotización rápida</span><h1>{form.id ? "Actualizar producto" : "Calcular precio"}</h1><p>Los precios cambian al instante mientras escribís.</p></div>{form.id && <button className="btn ghost small" onClick={() => setForm(EMPTY)}><Plus size={17} />Nuevo</button>}</header><form className="calculator" onSubmit={save}><section className="surface form-card">{form.id && <div className="edit-banner"><Pencil size={15} />Editando producto guardado</div>}
        ...
        <div className="two"><label className="field"><span>Precio de compra <em>*</em></span><div className="number-box"><i>$</i><input type="number" min="0" step="0.01" value={form.purchasePriceUsd} onChange={(e) => setForm({ ...form, purchasePriceUsd: e.target.value })} placeholder="0.00" required /></div><p className="hint">En dólares</p></label><label className="field"><span>Peso <em>*</em></span><div className="number-box"><input type="number" min="0.001" step="0.001" value={form.weightLb} onChange={(e) => setForm({ ...form, weightLb: e.target.value })} placeholder="0.00" required /><small>lb</small></div><p className="hint">Se suman {settings.extraWeightLb.toFixed(2)} lb.</p></label></div>
        {!form.name.trim() && (form.purchasePriceUsd || form.weightLb) && <p className="alert warning"><AlertCircle size={17} />Agregá el nombre para guardar.</p>}<button className="btn primary full" disabled={!validForm || saving}>{saving ? <Loader2 className="spin" /> : <Save />}{form.id ? "Guardar cambios" : "Guardar cotización"}</button>
      </section><div className="result-col"><Results price={Number.isFinite(price) ? price : 0} weight={Number.isFinite(weight) ? weight : 0} settings={settings} /><p className="formula-note"><Info />Tipo de cambio <b>₡{settings.exchangeRateCrc}</b> · Courier <b>{usd(settings.courierRateUsd)}/lb</b></p></div></form></div>}
      {tab === "products" && <div className="view"><header className="view-head"><span className="eyebrow">Historial guardado</span><h1>Productos</h1><p>Buscá por nombre, QR o código de barras.</p></header><section className="surface search-card"><div className="input-icon grow"><Search /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Buscar producto…" /></div><button className="scan-btn" onClick={() => setScanner(true)}><Camera /><span>Escanear</span></button></section>{loading ? <Empty icon={<Loader2 className="spin" />} title="Buscando…" text="" /> : !products.length ? <Empty icon={<PackageSearch />} title={query ? "No hay coincidencias" : "Todavía no hay productos"} text={query ? "Probá con otro nombre o escaneá el código." : "Las cotizaciones guardadas aparecerán aquí."} /> : <div className="product-grid">{products.map((product) => { const prices = calculatePrices(product.purchasePriceUsd, product.weightLb, settings); return <article className="product-card" key={product.id}><div className="product-title"><span className="avatar">{product.name[0].toUpperCase()}</span><div><h2>{product.name}</h2>{product.code && <small><ScanLine size={12} />{product.code}</small>}</div><button className="icon-btn" onClick={() => loadProduct(product)}><Pencil size={17} /></button></div><div className="facts"><div><span>Compra</span><b>{usd(product.purchasePriceUsd)}</b></div><div><span>Peso</span><b>{product.weightLb.toFixed(2)} lb</b></div><div className="green"><span>Venta GAM</span><b>{crc(prices.gamPriceCrc)}</b></div><div className="brown"><span>Venta Puerto</span><b>{crc(prices.puertoPriceCrc)}</b></div></div><button className="edit-link" onClick={() => loadProduct(product)}>Actualizar precio o peso <ChevronRight size={16} /></button></article>; })}</div>}</div>}
