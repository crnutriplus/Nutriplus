...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const row = await getD1().prepare("UPDATE products SET name=?,normalized_name=?,code=?,purchase_price_usd_cents=?,weight_milli_lb=?,quantity_available=?,minimum_stock=?,updated_at=CURRENT_TIMESTAMP WHERE id=? RETURNING *").bind(product.name, product.normalizedName, product.code, product.purchasePriceUsdCents, product.weightMilliLb, product.quantityAvailable, product.minimumStock, id).first();
