...
import { getCrmPanelOrder } from "@/lib/crm-panel";

function response(error: unknown) {
  const item = error instanceof CrmError ? error : new CrmError("No se pudo consultar el pedido.", 500, "CRM_PANEL_INTERNAL");
...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    return Response.json(
      await getCrmPanelOrder(db, access.context, (await context.params).id, configuredClient()),
    );
  } catch (error) {
