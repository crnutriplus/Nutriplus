...
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (!Number.isInteger(index) || index < 0) return Response.json({ error: "El archivo solicitado no es válido." }, { status: 400 });
    ...
    if (!file) return Response.json({ error: "No se encontró el archivo de la factura." }, { status: 404 });
