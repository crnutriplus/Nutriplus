...
type Expense = { id: string; date: string; category: string; categoryLabel: string; description: string; amountCrc: number; originalAmountMinor: number; exactOriginalAmount: number; currency: string; paymentMethod: string; provider: string | null; notes: string | null; routeId: string | null; orderId: string | null; sourceType: string; sourceId: string | null; entryType: string; businessScope: string; createdAt: string; invoice?: { documentId: string; provider: string; invoiceNumber: string | null; orderNumber: string | null; documentDate: string | null; currency: string | null; payment: { method: string; last4: string | null; amount: number; currency: string | null; cashAffecting: boolean } | null } | null };
type Sale = { id: string; orderId: string; orderNumber: string; customerName: string; deliveredAt: string; deliveredDate: string; routeId: string | null; productGross: number; discount: number; productNet: number; deliveryIncome: number; totalIncome: number; grossSales: number; returns: number; returnStatus: "NONE" | "PARTIALLY_RETURNED" | "RETURNED"; cogs: number | null; costStatus: string; grossProfit: number | null };
type FinanceData = {
  range: { from: string; to: string };
  metrics: { sales: number; grossSales: number; returns: number; netProfit: number | null; operatingExpenses: number; cashNet: number; grossProfit: number | null; cogs: number | null; knownCogs: number; deliveryIncome: number; deliveredOrders: number; marginPercent: number | null; costsComplete: boolean };
...
export function FinanceView({ active, onOpenOrder, onNavigateView, onNotify }: { active: boolean; onOpenOrder: (orderId: string) => void; onNavigateView: (view: View) => void; onNotify: (notice: Notice) => void }) {
  ...
  async function saveExpense(event: FormEvent, confirmDuplicate = false) {
    ...
    try {
      ...
      await api("/api/finance/expenses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
        ...
        amount: expense.amount, originalAmountMinor, currency: expense.currency, exchangeRateCrc: expense.currency === "USD" ? Number(expense.exchangeRate) : 1,
  ...
  async function openInvoice(expense: Expense) {
    const documentId = expense.invoice?.documentId;
    if (!documentId) return;
    const url = `/api/inventory-intake/${encodeURIComponent(documentId)}/file?index=0`;
    try {
      const response = await fetch(url, { method: "HEAD" });
      if (!response.ok) throw new Error("FACTURA_ARCHIVO_NO_DISPONIBLE");
      window.open(url, "_blank", "noopener");
    } catch {
      onNotify({ type: "error", text: "PROBLEMA: no se pudo abrir la factura. CAUSA: el archivo original ya no está disponible para este movimiento. QUÉ HACER: abrí el historial de Facturas o verificá el archivo asociado. ESTADO DE LOS DATOS: el gasto, sus pagos y su trazabilidad siguen guardados." });
    }
  }
  ...
  return <div className="view finance-view"><header className="view-head finance-head"><div><span className="eyebrow">Control financiero trazable</span><h1>Ventas y gastos</h1><p>Ventas solo al entregar; caja al cobrar o pagar. Cada cifra abre su origen.</p></div><div className="finance-export"><button className="btn secondary small" onClick={() => void download("excel")}><Download />Excel</button><button className="btn secondary small" onClick={() => void download("pdf")}><FileText />PDF</button></div></header>
    ...
    {view === "cash" && <><section className="finance-kpis secondary cash"><span><b>{crc(data.cash.incoming)}</b><small>Entradas reales</small></span><span><b>{crc(data.cash.outgoing)}</b><small>Salidas reales</small></span><span><b>{crc(data.cash.net)}</b><small>Flujo neto</small></span></section><section className="surface cash-methods"><h2>Por método</h2>{Object.entries(data.cash.methods).map(([method, item]) => <article key={method}><b>{method === "CASH" ? "Efectivo" : method === "SINPE" ? "SINPE" : method === "CARD" ? "Tarjeta" : "Otro"}</b><span>Entradas {crc(item.incoming)}</span><span>Salidas {crc(item.outgoing)}</span><strong>{crc(item.net)}</strong></article>)}</section><section className="surface finance-list"><header><h2>Movimientos de caja</h2></header>{[...data.cash.payments.map((item) => ({ id: item.id, date: item.createdAt, label: `${item.orderNumber} · ${item.customerName}`, detail: item.method, amount: item.signedAmount, original: null as string | null })), ...data.cash.expenses.map((item) => ({ id: item.id, date: item.date, label: `${item.description}${item.provider ? ` · ${item.provider}` : ""}`, detail: `${item.paymentMethod}${item.notes ? ` · ${item.notes}` : ""}`, amount: item.entryType === "REVERSAL" ? item.amountCrc : -item.amountCrc, original: item.currency === "USD" ? `USD ${item.exactOriginalAmount.toFixed(2)}` : null }))].sort((a, b) => b.date.localeCompare(a.date)).map((item) => <article key={item.id}><div><b>{item.label}</b><small>{item.date.replace("T", " ").slice(0, 16)} · {item.detail}</small></div><strong className={item.amount < 0 ? "negative" : "positive"}>{item.amount < 0 ? "-" : "+"}{item.original || crc(Math.abs(item.amount))}{item.original && <small> · {crc(Math.abs(item.amount))}</small>}</strong></article>)}</section></>}
