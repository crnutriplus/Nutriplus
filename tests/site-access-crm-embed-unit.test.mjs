...
test("embedded CRM gate exposes only intended routes",()=>{
  assert.equal(siteAccessDecision(req("/api/operations/crm-panel/embed/session",{method:"POST",headers:{origin:"https://nutriplus.test"}}),options).allowed,true);
  assert.equal(siteAccessDecision(req("/api/operations/crm-panel/embed/session",{method:"POST"}),options).allowed,false);
  assert.equal(siteAccessDecision(req("/api/operations/crm-panel/embed/session",{method:"POST",headers:{origin:"https://evil.test"}}),options).allowed,false);
  assert.deepEqual(siteAccessDecision(req("/operations/crm-panel/embed"),options),{allowed:true,reason:"crm_embed"});
  assert.equal(siteAccessDecision(req("/operations/crm-panel/embed",{method:"POST"}),options).allowed,false);
  assert.equal(siteAccessDecision(req("/operations/crm-panel/embed/"),options).allowed,false);
  assert.equal(siteAccessDecision(req("/operations/crm-panel/embed/extra"),options).allowed,false);
