...
test("Chatwoot client writes conversation custom attributes through the Chatwoot 4.17 scoped endpoint", async () => {
  ...
  await api.getConversation(1,9); await api.patchContactAttributes(1,2,{province:"Alajuela"}); await api.patchConversationAttributes(1,9,{order_status:"CONFIRMED"}); assert.equal(requests.length,3); assert.equal(requests[1][1].method,"PATCH"); assert.match(requests[1][0],/contacts\/2$/); assert.equal(requests[2][1].method,"POST"); assert.match(requests[2][0],/conversations\/9\/custom_attributes$/); assert.deepEqual(JSON.parse(requests[2][1].body),{custom_attributes:{order_status:"CONFIRMED"},merge:true});
