...
export async function createBackupForImport(db: D1Database, importId: number) {
  const existing = await db.prepare("SELECT id FROM import_backups WHERE import_id=? ORDER BY id LIMIT 1")
    .bind(importId).first<{ id: number }>();
  if (existing) return Number(existing.id);

  const backup = await db.prepare(`INSERT INTO import_backups (import_id,product_count,created_at)
    SELECT ?,COUNT(*),strftime('%Y-%m-%dT%H:%M:%fZ','now') FROM products RETURNING id`)
    .bind(importId).first<{ id: number }>();
